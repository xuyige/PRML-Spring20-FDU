...
def gen_data_batch2(length, batch_size):
    '''sample a mini-batch from the interval (start, end)
    Args :
    ...
    '''
    len = length - 1
    numbers_1 = [list(np.random.randint(0, 10, len)) for i in range(batch_size)]
    numbers_2 = [list(np.random.randint(0, 10, len)) for i in range(batch_size)]
    for i in range(batch_size):
        numbers_1[i].append(0)
        numbers_2[i].append(0)
    results = [[] for i in range(batch_size)]
    for i in range(batch_size):
        c = 0
        for j in range(length):
            results[i].append(numbers_1[i][j]+numbers_2[i][j]+c)
            if results[i][j] > 9:
                results[i][j] -= 10
                c = 1
            else:
                c = 0
    # for i in range(batch_size):
    #     print(numbers_1[i], numbers_2[i], results[i])
    return numbers_1, numbers_2, results

...
def gen_data_batch2(batch_size, len):
    '''sample a mini-batch from the interval (start, end)
    Args :
        batch_size: batch_size
        start: start number
    ...
    '''
    numbers_1 = [np.random.randint(0, 9, len-1).tolist() for i in range(batch_size)]
    numbers_2 = [np.random.randint(0, 9, len-1).tolist() for i in range(batch_size)]
    ...
    for i in range(batch_size):
        ...
        for j in range(len-1):
            results[i].append ( int(numbers_1[i][j]+numbers_2[i][j] + carry) )
            if results[i][j] >= 10:
                results[i][j] -= 10
                carry = 1
            else:
                carry = 0
        results[i].append ( carry )
    return numbers_1, numbers_2, results

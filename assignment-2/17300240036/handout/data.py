...
def get_big_number(length):
    string = ""
    for i in range(length):
        string += str(np.random.randint(1, 10))
    return int(string)

def transfer(nums, length):
    for i in range(len(nums)):
        nums[i] = list(str(nums[i]))
        nums[i].reverse()
        for j in range(len(nums[i])):
            nums[i][j] = int(nums[i][j])
        for j in range(length - len(nums[i])):
            nums[i].append(0)
            

def gen_big_data_batch(batch_size, length):
    numbers_1 = [get_big_number(length) for i in range(batch_size)]
    numbers_2 = [get_big_number(length) for i in range(batch_size)]
    results = [numbers_1[i] + numbers_1[i] for i in range(batch_size)]
    transfer(numbers_1, length + 1)
    transfer(numbers_2, length + 1)

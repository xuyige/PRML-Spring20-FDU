...
def trans(x, length):
    x = str(x)[::-1] + '0' * (length - len(str(x)))
    return list(map(int, list(x)))

def gen_data_batch_long(batch_size, length):
    start = 0
    end = 10 ** (length - 1) - 1

    numbers_1 = [random.randint(start, end) for i in range(batch_size)]
    numbers_2 = [random.randint(start, end) for i in range(batch_size)]
    results = [(numbers_1[i] + numbers_2[i]) for i in range(batch_size)]

    numbers_1 = [trans(numbers_1[i], length) for i in range(batch_size)]
    numbers_2 = [trans(numbers_2[i], length) for i in range(batch_size)]
    results = [trans(results[i], length) for i in range(batch_size)]

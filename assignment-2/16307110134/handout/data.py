...
def adv_gen_data_batch(batch_size, start, end):
    '''sample a mini-batch from the interval (start, end)
    Args :
        batch_size: batch_size
        start: start number
    ...
    '''
    numbers_1 = np.array([random.randrange(start,end) for _ in range(batch_size)])
    numbers_2 = np.array([random.randrange(start,end) for _ in range(batch_size)])
    results = numbers_1 + numbers_2
    return numbers_1, numbers_2, results
...
def convertNum2Digits(Num):
    ...
    for o in chNums:
        if o == '-':
            print(Num)
    digitNums = [int(o) for o in chNums]
    return digitNums
